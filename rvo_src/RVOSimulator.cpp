...
namespace RVO {
	...
	size_t RVOSimulator::addAgentCoordinates(const float pos_x, const float pos_y)
	{
		return addAgent(Vector2(pos_x, pos_y));
	}
	...
	size_t RVOSimulator::addAgentExt(const float pos_x, const float pos_y,
		float neighborDist,
		size_t maxNeighbors, float timeHorizon,
		float timeHorizonObst, float radius, float maxSpeed)
	{
		return addAgent(Vector2(pos_x, pos_y), neighborDist, maxNeighbors, timeHorizon, timeHorizonObst, radius, maxSpeed);
	}
	...
	size_t RVOSimulator::addObstacleCoordinates(const std::vector<float>& vertices)
	{
		std::vector<Vector2> vs(vertices.size() / 2);
		for (size_t i = 0; i < vs.size(); i++) {
			vs[i] = Vector2(vertices[2*i], vertices[2*i+1]);
		}
		return addObstacle(vs);
	}
...
#pragma omp parallel for
	...
	bool RVOSimulator::queryVisibilityCoordinates(const float point1_x, const float point1_y,
												  const float point2_x, const float point2_y) const
	{
		return queryVisibility(Vector2(point1_x, point1_y), Vector2(point2_x, point2_y));
	}
	...
	void RVOSimulator::setAgentPositionCoordinates(size_t agentNo, const float position_x, const float position_y)
	{
		setAgentPosition(agentNo, Vector2(position_x, position_y));
	}
	...
	void RVOSimulator::setAgentPrefVelocityCoordinates(size_t agentNo, const float prefVelocity_x, const float prefVelocity_y)
	{
		setAgentPrefVelocity(agentNo, Vector2(prefVelocity_x, prefVelocity_y));
	}
	...
	void RVOSimulator::setAgentVelocityCoordinates(size_t agentNo, const float velocity_x, const float velocity_y)
	{
		setAgentVelocity(agentNo, Vector2(velocity_x, velocity_y));
	}
